...
void matrix::AllocMem(void)
{
	...
	if (!ptr) throw std::runtime_error("matrix::AllocMem(): Error!");

	for (unsigned int i = 0; i < this->n; i++) {
		this->ptr[i] = new double[this->m];

		if (!ptr[i]) throw std::runtime_error("matrix::AllocMem(): Error!");
...
{
	if (this->n != mt.n || this->m != mt.m) throw std::runtime_error(" matrix::operator+ : Error! ");
...
matrix matrix::operator*(const matrix& mt) const
{
	...
	if (this->m != mt.n) throw std::runtime_error(" matrix::operator* : Error! " );
...
matrixC matrix::operator*(const matrixC& mt) const
{

	if (this->m != Str(mt)) throw std::runtime_error(" matrix::operator* : Error! " );

	matrixC rez(this->n, Col(mt));
	for (unsigned int i = 0; i < Str(rez); i++)
		for (unsigned int j = 0; j < Col(rez); j++) {
			rez[i][j] = zeroComplex();
			for (unsigned int k = 0; k < this->m; k++) rez[i][j] += this->ptr[i][k] * mt[k][j];
		}
	return rez;
...
matrix matrix::operator/(double x) const
{
	...
	if (x + 1 == 1) throw std::runtime_error(" matrix::operator/ : Error! " );
...
void matrix::show() //const
{
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < m; j++)
		{
			cout<< scientific << setw(15) << ptr[i][j]<<"  ";
...
void matrixC::AllocMem(void)
{
	...
	if (!ptr) throw std::runtime_error("matrixC::AllocMem(): Error!" );

	for (unsigned int i = 0; i < this->n; i++) {
		this->ptr[i] = new complex[this->m];

		if (!ptr[i]) throw std::runtime_error("matrixC::AllocMem(): Error!" );
...
matrixC matrixC::operator+(const matrixC& mt) const
{
	...
	if (this->n != mt.n || this->m != mt.m) throw std::runtime_error(" matrixC::operator+ : Error! " );
...
matrixC matrixC::operator*(const matrixC& mt) const
{
	...
	if (this->m != mt.n) throw std::runtime_error(" matrixC::operator* : Error! " );
...
matrixC matrixC::operator*(const matrix& mt) const
{
	if (this->m != Str(mt)) throw std::runtime_error(" matrixC::operator* : Error! " );
...
matrixC matrixC::operator/(const complex& z) const
{
	...
	if (z + 1 == 1) throw std::runtime_error(" matrixC::operator/ : Error! " );
