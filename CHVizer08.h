/*
*In this version, users can zoom in/out the visualization by scrolling the mouse wheel.
*20250613
*/
#pragma once
#include <gl/glut.h>
...
public:
    ...
    void initialize(const std::vector<vec3>& convex_hull_vertices) {
    ...
    void display() {
        ...
        for (size_t i = 0; i < vertices.size() / 3; i++) {
            ...
            vec3 v1 = vertices[3 * i + 1] - vertices[3 * i];
            vec3 v2 = vertices[3 * i + 2] - vertices[3 * i];
    ...
    static void run(int argc, char** argv,
        const std::vector<vec3>& convex_hull_vertices) {
        glutInit(&argc, argv);
        glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH | GLUT_MULTISAMPLE);
        glutInitWindowSize(800, 600);
        glutCreateWindow("3D Convex Hull Visualizer");

        static ConvexHullVisualizer visualizer;
        visualizer.initialize(convex_hull_vertices);
