...
public:
	...
	matrix() : n(0), m(0), ptr(nullptr) {}//new added by Mu
	//Default constructor (must exist, otherwise it cannot be used in the container - vector)
	...
	matrix	operator*(const matrix&) const;
	matrixC operator*(const matrixC&) const;//added by Mu
	...
	*/
	...
	{
		if (Str(M) != 3 || Col(M) != 3) {
			throw std::invalid_argument("Matrix must be 3x3 for multiplication with vec3.");
		}
...
public:
	...
	matrixC		operator*(const matrix&) const;//added by Mu
	matrixC		operator*(const matrixC&) const;
