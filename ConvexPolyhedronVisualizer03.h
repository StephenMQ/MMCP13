...
* Modified to run GLUT in a separate thread.
* 20250616
*/
#pragma once
#include <gl/glut.h>
...
private:
    ...
    // Callback wrappers
    static std::shared_ptr<ConvexPolyhedronVisualizer> visualizerInstance;
...
public:
    ...
    static void run(int argc, char** argv, const std::vector<Face>& convex_poly_faces) {
        if (running) return;

        running = true;
        initialized = false;

        renderThread = std::thread([argc, argv, convex_poly_faces]() {
            auto faces_copy = convex_poly_faces;

            char fake_argv0[] = "Visualizer";
            ...
            visualizerInstance = std::make_shared<ConvexPolyhedronVisualizer>();
            visualizerInstance->initialize(faces_copy);
...

        {
            ...
            initCV.wait(lock, [] { return initialized; });
        }
...
#ifdef _WIN32
            HWND hwnd = FindWindowA(NULL, "3D Convex Polyhedron Visualizer");
            if (hwnd) {
...
std::atomic<bool> ConvexPolyhedronVisualizer::running(false);
std::thread ConvexPolyhedronVisualizer::renderThread;
std::mutex ConvexPolyhedronVisualizer::initMutex;
std::condition_variable ConvexPolyhedronVisualizer::initCV;
bool ConvexPolyhedronVisualizer::initialized(false);
std::shared_ptr<ConvexPolyhedronVisualizer> ConvexPolyhedronVisualizer::visualizerInstance;

// Static callback wrappers
void ConvexPolyhedronVisualizer::displayWrapper() {
    if (visualizerInstance) visualizerInstance->display();
}

void ConvexPolyhedronVisualizer::reshapeWrapper(int w, int h) {
    if (visualizerInstance) visualizerInstance->reshape(w, h);
}

void ConvexPolyhedronVisualizer::mouseWrapper(int button, int state, int x, int y) {
    if (visualizerInstance) visualizerInstance->mouse(button, state, x, y);
}

void ConvexPolyhedronVisualizer::motionWrapper(int x, int y) {
    if (visualizerInstance) visualizerInstance->motion(x, y);
}